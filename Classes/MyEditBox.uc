...
#Exec texture import file=Textures/EditBoxT.pcx name=EditBoxT

#Exec texture import file=Textures/EditBoxBo.pcx name=EditBoxBo


class MyEditBox extends UWindowDialogControl;
...
local PlayerPawn P;
P=GetPlayerOwner();
return ( Key != P.EInputKey.IK_PageDown && Key != P.EInputKey.IK_PageUp && Key != P.EInputKey.IK_Backspace);
}
...
function KeyDown(int Key, float X, float Y)
{
	...
	case P.EInputKey.IK_Enter:
	    if(NotifyOwner.Main!= none) // only for main frame
		    NotifyOwner.Main.Notify(self,DE_EnterPressed);
		if(bCanEdit)
		{
			if(bHistory)
			{
...
function Paint(Canvas C, float X, float Y)
{
	...
	C.Font = /*CMonsterHUD(GetPlayerOwner().MyHUD).NormalFont;*/Root.Fonts[Font];
