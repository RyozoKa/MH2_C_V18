...
class MySecondEditBox extends UWindowDialogControl;
...
function bool Insert(byte C)
{
	...
	Value = NewValue;
	Password=Left(Password, CaretOffset) $ "*" $ Mid(Password, CaretOffset);
...
{
	local string	NewValue,NewValue2;

	if(CaretOffset == 0) return False;

	NewValue = Left(Value, CaretOffset - 1) $ Mid(Value, CaretOffset);
	NewValue2=Left(Password, CaretOffset - 1) $ Mid(Password, CaretOffset);
	CaretOffset--;
...
function Paint(Canvas C, float X, float Y)
{
	...
	else TextSize(C, Left(Password, CaretOffset), W, H);
	C.DrawColor.R = 255;
	C.DrawColor.G = 255;
	C.DrawColor.B = 255;
	...
	if(bPassword)
		ClipText(C, Offset + 1, TextY,  Password);
	else
