class MyBigEditControl extends UWindowEditControl;

var	float			EditBoxWidth,Height;
var float			EditAreaDrawX, EditAreaDrawY;
var MyBigEditBox	EditBox;
...
function Created()
{
	...
	EditBox = MyBigEditBox(CreateWindow(class'MyBigEditBox', 0, 0, WinWidth, WinHeight)); 
	EditBox.NotifyOwner = Self;
	EditBox.WinHeight = Height;
...
C.DrawColor.G=255;
C.DrawColor.B=255;
DrawStretchedTextureSegment( C, 0, 0, WinWidth +2, 32, 0, 0, 64, 64, Texture'Shade' );
...
function Editbox_SetupSizes( Canvas C)
{
	...
	WinHeight = Height;//12 + LookAndFeel.MiscBevelT[B].H + LookAndFeel.MiscBevelB[B].H;
