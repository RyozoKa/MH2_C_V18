//=============================================================================
// BiterFish.
// Do not add directly - rather add BiterFishSchools to the world - LOL F.. Y.. so much
//=============================================================================
//I think this class can be really Awesome
class BiterFish2 extends FlockPawn;
...
var BiterFishSchool2 School;
var texture FishSkins[6];

function PostBeginPlay()
{
	School = BiterfishSchool2(Owner); 
	Super.PostBeginPlay();
	if ( School == None )
	{
		destroy();
...
Auto State Swimming
{
	...
	{
		if ( School.IsInState('Stasis')) /*&& !PlayerCanSeeMe() No craps is a default rule, :P  )*/
		{
			School.Remove(self);
			return;
		}
...
State Dying
{
	...
	function Timer()
	{
//		if ( !PlayerCanSeeMe() ) LOL, I already keept a silent moment enjoying your death ...
			Destroy();
	}
...
//Yea, Reduced Damage ? No craps, I told you :P, go to your home and cry, Kira can use RocketLauncher now as well :P
